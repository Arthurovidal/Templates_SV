...
export async function POST(request: Request) {
  ...
  // ── 5. Envoi des emails ──────────────────────────────────
  const siteName = process.env['NEXT_PUBLIC_CORPORATE_SITE_NAME'] ?? 'Cabinet';
  const siteUrl = process.env['NEXT_PUBLIC_CORPORATE_SITE_URL'] ?? 'http://localhost:3001';
  const toEmail = process.env['CORPORATE_CONTACT_EMAIL_TO'];

  if (!toEmail) {
    console.error('[Contact] CORPORATE_CONTACT_EMAIL_TO manquant');
    return NextResponse.json(
  ...
  const notificationResult = await sendEmail({
    ...
    replyTo: data.email,
    templateId: 'corporate-notification',
  });
  ...
  sendEmail({
    ...
    templateId: 'corporate-confirmation',
  }).catch((err) => {
