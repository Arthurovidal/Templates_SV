...
export async function POST(request: Request) {
  ...
  // ── 5. Envoi des emails ──────────────────────────────────
  const siteName = process.env['NEXT_PUBLIC_LIFESTYLE_SITE_NAME'] ?? 'Cabinet';
  const siteUrl = process.env['NEXT_PUBLIC_LIFESTYLE_SITE_URL'] ?? 'http://localhost:3001';
  const toEmail = process.env['LIFESTYLE_CONTACT_EMAIL_TO'];

  if (!toEmail) {
    console.error('[Contact] LIFESTYLE_CONTACT_EMAIL_TO manquant');
    return NextResponse.json(
  ...
  const notificationResult = await sendEmail({
    ...
    replyTo: data.email,
    templateId: 'lifestyle-notification',
  });
  ...
  sendEmail({
    ...
    templateId: 'lifestyle-confirmation',
  }).catch((err) => {
