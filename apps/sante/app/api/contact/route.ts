...
export async function POST(request: Request) {
  ...
  // ── 5. Envoi des emails ──────────────────────────────────
  const siteName = process.env['NEXT_PUBLIC_SANTE_SITE_NAME'] ?? 'Cabinet';
  const siteUrl = process.env['NEXT_PUBLIC_SANTE_SITE_URL'] ?? 'http://localhost:3001';
  const toEmail = process.env['SANTE_CONTACT_EMAIL_TO'];

  if (!toEmail) {
    console.error('[Contact] SANTE_CONTACT_EMAIL_TO manquant');
    return NextResponse.json(
